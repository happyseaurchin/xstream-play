...
- **`ticketing-agent`** (sibling repo at `~/Projects/ticketing-agent/`) — reference issuer + verifier daemon. M0 (skeleton), M1 (envelope helpers + grain establish/walk/revoke against the live MCP), M2 (catalogue route), M3 (Stripe driver + purchase flow + rate limits), and M4 (verifier daemon + audit log) have all landed. M5 (Stripe refund + grain revoke) is the next milestone. ~5 days total per spec §8.
...
- **Verification envelopes live in the verifier's own audit log, NOT on the registration position.** The verifier writes to `sed:<verifier-bare-id>-audit-<yyyy-mm>` (one collective per calendar month) on its own beach. Each entry is a `pscale_register` whose declaration is the envelope: `[ticket-verified by=<verifier_id> at=<iso8601> registration=<sed:collective:position> grain=<grain:pair_id:side>]` (analogous for `ticket-rejected ... reason=...` and `ticket-expired ...`). To check status for a given registration, walk the verifier's audit collective and match on `registration=<your sed:collective:position>`. Across month boundaries, walk current and previous month. **Why not the registration itself?** Because the registrant's `sed:` position is write-locked to the registrant's passphrase, and the lock covers digit children. A foreign agent (the verifier) cannot write into the registrant's position without a substrate change — so verification decisions go to the verifier's own beach. M4 of ticketing-agent has landed and is writing real envelopes to the audit log; you can read them now without hand-writing.
...
6. **Watch the verifier's audit collective for a matching envelope.** Walk `sed:<_tickets.verifier-bare-id>-audit-<yyyy-mm>` (current month, plus previous if needed across boundaries) and find the entry whose declaration is a `[ticket-verified ...]`, `[ticket-rejected ...]`, or `[ticket-expired ...]` envelope with `registration=sed:<your-collective>:<your-position>` matching the registration just performed. M4 of ticketing-agent has landed — real envelopes are being written to the audit log; no hand-writing needed for end-to-end test.
...
- **M3 + M4 have landed** in ticketing-agent. Full end-to-end is now possible: surface buy → Stripe Checkout → real grain arrives → Step A registration → verifier daemon walks the registration → `[ticket-verified]` envelope appears in the verifier's audit collective → xstream-play polls the audit collective and unlocks affordances. M5 (Stripe refund + grain revoke) is the next milestone; the verifier already picks up `[ticket-revoked]` envelopes on grains and emits `[ticket-rejected]` audit entries on the next tick.
